...
class EpsDriverNode : public ff_util::FreeFlyerNodelet {
 ...
 protected:
  ...
  void TelemetryCallback(const ros::TimerEvent &) {
    ...
    if (en_pub_housekeeping_) {
      ...
      if (eps_->GetHousekeeping(EPS::EVERYTHING, data)) {
        ...
        keys["Analog GND 1 voltage"] = data[EPS::HK_AGND1_V];
        keys["Supply voltage"] = data[EPS::HK_SUPPLY_IN_V];
        keys["Payload 3 current"] = data[EPS::HK_PAYLOAD_PWR3_I];
        keys["Subsystem 1,1 voltage"] = data[EPS::HK_SUBSYS1_1_PWR_V];
        keys["Subsystem 1,2 voltage"] = data[EPS::HK_SUBSYS1_2_PWR_V];
        keys["Unregulated voltage"] = data[EPS::HK_UNREG_V];
        keys["System current"] = data[EPS::HK_SYSTEM_I];
        keys["Battery 4 voltage"] = data[EPS::HK_BAT4V_V];
        keys["Battery 3 voltage"] = data[EPS::HK_BAT3V_V];
        keys["Battery 2 voltage"] = data[EPS::HK_BAT2V_V];
        keys["Battery 1 voltage"] = data[EPS::HK_BAT1V_V];
        keys["Supply current"] = data[EPS::HK_SUPPLY_I];
        keys["5V live voltage"] = data[EPS::HK_5VLIVE_V];
        keys["Analog GND 2 voltage"] = data[EPS::HK_AGND2_V];
        keys["Fan current"] = data[EPS::HK_FAN_PWR_I];
        keys["Auxiliary current"] = data[EPS::HK_AUX_PWR_I];
        keys["Payload 4 current"] = data[EPS::HK_PAYLOAD_PWR4_I];
        keys["Payload 2 current"] = data[EPS::HK_PAYLOAD_PWR2_I];
        keys["Payload 1 current"] = data[EPS::HK_PAYLOAD_PWR1_I];
        keys["5A regulator 1 current"] = data[EPS::HK_5A_REG1_PWR_I];
        keys["PMC 1 (right) current"] = data[EPS::HK_MOTOR1_I];
        keys["Subsystem 2 voltage"] = data[EPS::HK_SUBSYS2_PWR_V];
        keys["PMC 2 (left) current"] = data[EPS::HK_MOTOR2_I];
        keys["5A regulator 2 current"] = data[EPS::HK_5A_REG2_PWR_I];
        keys["5A regulator 3 current"] = data[EPS::HK_5A_REG3_PWR_I];
        keys["Main 5 current"] = data[EPS::HK_MAIN5_PWR_I];
        keys["AUO current"] = data[EPS::HK_AUO_PWR_I];
        keys["HLP current"] = data[EPS::HK_HLP_I];
        keys["Universal serial bus current"] = data[EPS::HK_USB_PWR_I];
        keys["LLP current"] = data[EPS::HK_LLP_I];
        keys["MLP current"] = data[EPS::HK_MLP_I];
        keys["Ethernet current"] = data[EPS::HK_ENET_PWR_I];
        std::map<std::string, double>::iterator it;
        // clear previous vector of channels
        msg.channels.clear();
        for (it = keys.begin(); it != keys.end(); it++) {
          static ff_hw_msgs::ChannelState channel;
          channel.name = it->first;
          channel.value = static_cast<float>(it->second);
  ...
  bool PayloadConfigureCallback(
    ...
    if (req.top_front == on) bat[true] |= (1 << EPS::CHANNEL_PAYLOAD_EN1);
    if (req.top_front == off) bat[false] |= (1 << EPS::CHANNEL_PAYLOAD_EN1);
    if (req.bottom_front == on) bat[true] |= (1 << EPS::CHANNEL_PAYLOAD_EN2);
    if (req.bottom_front == off) bat[false] |= (1 << EPS::CHANNEL_PAYLOAD_EN2);
    if (req.top_aft == on) bat[true] |= (1 << EPS::CHANNEL_PAYLOAD_EN3);
    if (req.top_aft == off) bat[false] |= (1 << EPS::CHANNEL_PAYLOAD_EN3);
    if (req.bottom_aft == on) bat[true] |= (1 << EPS::CHANNEL_PAYLOAD_EN4);
    if (req.bottom_aft == off) bat[false] |= (1 << EPS::CHANNEL_PAYLOAD_EN4);
    // Apply the operation in batches
...
PLUGINLIB_DECLARE_CLASS(eps_driver, EpsDriverNode, eps_driver::EpsDriverNode,
                        nodelet::Nodelet);
