...
bool EPS::SetLeds(uint32_t mask, LedMode const value) {
  ...
  if (mask & (1 << LED_SA2)) chanmask |= (1 << CHANNEL_STATUSA2_LED);
  if (mask & (1 << LED_SA1)) chanmask |= (1 << CHANNEL_STATUSA1_LED);
  if (mask & (1 << LED_SB2)) chanmask |= (1 << CHANNEL_STATUSB2_LED);
  if (mask & (1 << LED_SB1)) chanmask |= (1 << CHANNEL_STATUSB1_LED);
  if (mask & (1 << LED_SC2)) chanmask |= (1 << CHANNEL_STATUSC2_LED);
  if (mask & (1 << LED_SC1)) chanmask |= (1 << CHANNEL_STATUSC1_LED);
  if (mask & (1 << LED_VIDEO)) chanmask |= (1 << CHANNEL_VIDEO_LED);
  if (mask & (1 << LED_AUDIO)) chanmask |= (1 << CHANNEL_AUDIO_LED);
  if (mask & (1 << LED_LIVE)) chanmask |= (1 << CHANNEL_LIVE_LED);
  uint8_t cmd[9] = {
