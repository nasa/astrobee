...
OpState* OpStateReady::HandleCmd(ff_msgs::CommandStampedPtr const& cmd) {
  ...
  if (cmd->cmd_name == CommandConstants::CMD_NAME_SET_PLAN) {
    ...
    if (exec_->StartAction(SWITCH, cmd->cmd_id, err_msg)) {
  ...
  // Below are all the commands used to configure mobility
    ...
    if ((exec_->GetMobilityState().state == ff_msgs::MobilityState::STOPPING &&
      ...
      if (!exec_->ConfigureMobility(cmd->cmd_id, err_msg)) {
        return this;
      }

      if (exec_->StartAction(MOVE, cmd->cmd_id, err_msg)) {
        return OpStateRepo::Instance()->teleop()->StartupState();
      }
    } else {
      exec_->PublishCmdAck(cmd->cmd_id,
    ...
    if ((exec_->GetMobilityState().state == ff_msgs::MobilityState::STOPPING &&
      ...
      exec_->PublishCmdAck(cmd->cmd_id,
                           ff_msgs::AckCompletedStatus::EXEC_FAILED,
                           "Must be stopped or drifting before auto returning");
      ROS_ERROR("Executive: Must be stopped or drifting before auto returning");
    }
