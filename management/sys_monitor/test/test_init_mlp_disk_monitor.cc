...
TEST(init_mlp_disk_monitor, MlpDiskMonitorInitFailed) {
  ...
  if (!fault_config.CheckValExists("mlp_disk_monitor")) {
    ROS_ERROR("Couldn't extract mlp disk monitor fault need for init fault id");
    // Cause test to fail
    EXPECT_EQ(2, 1);
    return;
  }
  ...
  for (int i = 1; i < (fault_table.GetSize() + 1); i++) {
    ...
    if (!fault_entry.GetStr("key", &fault_key)) {
      ROS_ERROR("Fault key at index %i not in mlp disk monitor fault table", i);
      // Cause test to fail
      EXPECT_EQ(2, 1);
      return;
    }
    ...
    if (fault_key == "INITIALIZATION_FAILED") {
      if (!fault_entry.GetInt("id", &init_fault_id)) {
        ROS_ERROR("Fault id for mlp disk monitor init failed fault missing.");
        // Cause test to fail
        EXPECT_EQ(2, 1);
        return;
      }
  ...
  if (init_fault_id == -1) {
    ROS_ERROR("Unable to extract init fault id for mlp disk monitor.");
    // Cause test to fail
    EXPECT_EQ(2, 1);
    return;
  }
  ...
  // Start timer to check if the mlp disk monitor fault has been thrown.
