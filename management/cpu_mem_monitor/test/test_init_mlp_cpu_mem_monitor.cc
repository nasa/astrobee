...
TEST(init_cpu_mem_monitor, CpuMemMonitorInitFailed) {
  ...
  if (!fault_config.CheckValExists("mlp_cpu_mem_monitor")) {
    ROS_ERROR("Couldn't extract mlp cpu memory monitor fault. Need for init fault id");
    // Cause test to fail
    EXPECT_EQ(2, 1);
    return;
  }
  ...
  for (int i = 1; i < (fault_table.GetSize() + 1); i++) {
    ...
    if (!fault_entry.GetStr("key", &fault_key)) {
      ROS_ERROR("Fault key at index %i not in mlp cpu memory monitor fault table.", i);
      // Cause test to fail
      EXPECT_EQ(2, 1);
      return;
    }
    ...
    if (fault_key == "INITIALIZATION_FAILED") {
      if (!fault_entry.GetInt("id", &init_fault_id)) {
        ROS_ERROR("Fault id for mlp cpu memory monitor init failed fault missing.");
        // Cause test to fail
        EXPECT_EQ(2, 1);
        return;
      }
  ...
  if (init_fault_id == -1) {
    ROS_ERROR("Unable to extract init fault id for mlp cpu memory monitor.");
    // Cause test to fail
    EXPECT_EQ(2, 1);
    return;
  }
