...
TEST(init_llp_cpu_monitor, LlpCpuMonitorInitFailed) {
  ...
  for (int i = 1; i < (fault_table.GetSize() + 1); i++) {
    ...
    if (!fault_entry.GetStr("key", &fault_key)) {
      ROS_ERROR("Fault key at index %i not in llp cpu monitor fault table.", i);
      // Cause test to fail
      EXPECT_EQ(2, 1);
      return;
    }
    ...
    if (fault_key == "INITIALIZATION_FAILED") {
      if (!fault_entry.GetInt("id", &init_fault_id)) {
        ROS_ERROR("Fault id for llp cpu monitor init failed fault missing.");
        // Cause test to fail
        EXPECT_EQ(2, 1);
        return;
      }
  ...
  if (init_fault_id == -1) {
    ROS_ERROR("Unable to extract init fault id for llp cpu monitor.");
    // Cause test to fail
    EXPECT_EQ(2, 1);
    return;
  }
  ...
  // Start timer to check if the llp cpu monitor fault has been thrown.
