...
def generate_test_description():
    test_ff_action_deadline_timeout = Node(
        executable=launch.substitutions.PathJoinSubstitution(
            [
                launch.substitutions.LaunchConfiguration("test_binary_dir"),
                "test_ff_action_deadline_timeout",
            ]
        ),
        output="screen",
    )
    ...
    return launch.LaunchDescription(
        ...
        "test_ff_action_deadline_timeout": test_ff_action_deadline_timeout,
    }


class TestTerminatingProcessStops(unittest.TestCase):
    def test_proc_terminates(self, proc_info, test_ff_action_deadline_timeout):
        proc_info.assertWaitForShutdown(process=test_ff_action_deadline_timeout, timeout=30)
