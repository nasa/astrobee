...
def generate_test_description():
    test_ff_action_preempt_own_goal = Node(
        executable=launch.substitutions.PathJoinSubstitution(
            [
                launch.substitutions.LaunchConfiguration("test_binary_dir"),
                "test_ff_action_preempt_own_goal",
            ]
        ),
        output="screen",
    )
    ...
    return launch.LaunchDescription(
        ...
        "test_ff_action_preempt_own_goal": test_ff_action_preempt_own_goal,
    }


class TestTerminatingProcessStops(unittest.TestCase):
    def test_proc_terminates(self, proc_info, test_ff_action_preempt_own_goal):
        proc_info.assertWaitForShutdown(process=test_ff_action_preempt_own_goal, timeout=30)
