...
def generate_test_description():
    ...
    return launch.LaunchDescription([
        ...
        test_ff_service,
        # TimerAction(period=2.0, actions=[test_ff_service]),
...
class TestGTestWaitForCompletion(unittest.TestCase):
    ...
    def test_gtest_run_complete(self, test_ff_service):
        self.proc_info.assertWaitForShutdown(test_ff_service, timeout=4000.0)


@launch_testing.post_shutdown_test()
class TestGTestProcessPostShutdown(unittest.TestCase):
    # Checks if the test has been completed with acceptable exit codes
    def test_gtest_pass(self, proc_info, test_ff_service):
        launch_testing.asserts.assertExitCodes(
            proc_info, process=test_ff_service
        )
