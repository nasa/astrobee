...
def generate_test_description():
    test_ff_action_nominal_behavior = Node(
        executable=launch.substitutions.PathJoinSubstitution(
            [
                launch.substitutions.LaunchConfiguration("test_binary_dir"),
                "test_ff_action_nominal_behavior",
            ]
        ),
        output="screen",
    )
    ...
    return launch.LaunchDescription(
        ...
        "test_ff_action_nominal_behavior": test_ff_action_nominal_behavior,
    }


class TestTerminatingProcessStops(unittest.TestCase):
    def test_proc_terminates(self, proc_info, test_ff_action_nominal_behavior):
        proc_info.assertWaitForShutdown(process=test_ff_action_nominal_behavior, timeout=30)
