...
TEST(KeepoutIO, LoadBadKeepout) {
  ...
  EXPECT_FALSE(jsonloader::ReadKeepoutFile(kDataDir + "keepout_missing_safe.json", &k));

  LOG(INFO) << "expect error about 'sequence' field";
  EXPECT_FALSE(jsonloader::ReadKeepoutFile(kDataDir + "keepout_missing_sequence.json", &k));

  LOG(INFO) << "expect error about 'invalid bounding box'";
  EXPECT_FALSE(jsonloader::ReadKeepoutFile(kDataDir + "keepout_wrong_sequence.json", &k));
}
...
TEST(KeepoutIO, LoadDir) {
  ...
  jsonloader::ReadKeepoutDirectory(kDataDir + "group/", &safeZone, &dangerZone);
