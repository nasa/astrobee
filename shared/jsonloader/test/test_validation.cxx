...
TEST(Validation, ObjectField) {
  ...
  ObjectField good("parent", {new StringField("child", "value")});
  EXPECT_TRUE(good.Validate(v));

  ObjectField bad("parent", {new Field("non_child", Json::nullValue)});
  EXPECT_FALSE(bad.Validate(v));

  ObjectField optional("parent", {new Field("non_child", Json::nullValue, false)});
  ObjectField optional_obj("404", {new Field("not_found", Json::nullValue)}, false);
  EXPECT_TRUE(optional.Validate(v));
