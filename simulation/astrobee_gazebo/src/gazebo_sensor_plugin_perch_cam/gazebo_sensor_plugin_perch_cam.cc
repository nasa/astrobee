...
class GazeboSensorPluginPerchCam : public FreeFlyerSensorPlugin {
 public:
  GazeboSensorPluginPerchCam() : FreeFlyerSensorPlugin(NODE_PERCH_CAM) {}
 ...
 protected:
  ...
  void LoadCallback(ros::NodeHandle *nh, sensors::SensorPtr sensor, sdf::ElementPtr sdf) {
    ...
    if (!sensor_) {
      gzerr << "GazeboSensorPluginPerchCam requires a camera sensor as a parent.\n";
      return;
    }
    ...
    update_ = sensor_->DepthCamera()->ConnectNewRGBPointCloud(
      boost::bind(&GazeboSensorPluginPerchCam::Callback, this, _1, _2, _3, _4, _5));
  }
