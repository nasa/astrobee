    ...
    rate_(10.0), distance_(0.05), delay_(5.0), lock_(false) {
      ...
      fsm_.Add(UNKNOWN, SENSE_NEAR | SENSE_FAR,
        [this](FSM::Event const& event) -> FSM::State {
          if (event == SENSE_FAR)
            return UNDOCKED;
          // Calculate the force to the newarest berth
          lock_ = true;
      ...
      fsm_.Add(DOCKING, TIMEOUT,
        [this](FSM::Event const& event) -> FSM::State {
          // Disable the physics engine to prevent instability
          lock_ = true;
          // We are not docked
          return DOCKED;
        });
      ...
      fsm_.Add(DOCKED, SENSE_FAR,
        [this](FSM::Event const& event) -> FSM::State {
          // Null the force to allow escape
          lock_ = true;
          // Stop the timer that signals successfult connection
          timer_delay_.stop();
 ...
 protected:
  ...
  void LoadCallback(ros::NodeHandle *nh,
    ...
    pub_ = nh->advertise<ff_hw_msgs::EpsDockStateStamped>(
      TOPIC_HARDWARE_EPS_DOCK_STATE, 100);
  ...
  void BerthCallback() {
    ...
    try {
      ...
      event::Events::DisconnectWorldUpdateEnd(connection_);
      // Start an update check for force application
      connection_ = event::Events::ConnectWorldUpdateBegin(
        std::bind(&GazeboModelPluginEps::LockCallback, this));
      // Once we have berth locations start timer for checking dock status
      timer_update_.start();
  ...
  // Apply a magnetic force to the platform
  void LockCallback() {
    // The magnetic would better be modeled by d^3 path loss, However, for the
    // sake of simplicity, and because the distance is very small, I'm just
    // going to assume a constant linear force exerted on the robot when it
    // comes within the threshold distance of the dock.
    if (lock_ && nearest_ != berths_.end())
      GetModel()->SetWorldTwist(math::Vector3(0, 0, 0), math::Vector3(0, 0, 0));
  }
