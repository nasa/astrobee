...
GncBlowersAutocode::GncBlowersAutocode(void) : b1(true), b2(false) {
...
void GncBlowersAutocode::Step(void) {
  ...
  omega << states_[0].omega_B_ECI_B[0], states_[0].omega_B_ECI_B[1], states_[0].omega_B_ECI_B[2];
  Eigen::Matrix<float, 6, 1> servo_cmd;
  servo_cmd << states_[0].servo_cmd[0], states_[0].servo_cmd[1], states_[0].servo_cmd[2],
               states_[0].servo_cmd[3], states_[0].servo_cmd[4], states_[0].servo_cmd[5];
  b1.Step(states_[0].impeller_cmd, servo_cmd, omega, states_[0].battery_voltage);
  servo_cmd << states_[1].servo_cmd[0], states_[1].servo_cmd[1], states_[1].servo_cmd[2],
               states_[1].servo_cmd[3], states_[1].servo_cmd[4], states_[1].servo_cmd[5];
  b2.Step(states_[1].impeller_cmd, servo_cmd, omega, states_[1].battery_voltage);
  ...
  Blower b[2] = {b1, b2};
  for (int i = 0; i < 2; i++) {
    TestTwoVectors("force", b[i].Force(), states_[i].force_B, 0.001);
    TestTwoVectors("torque", b[i].Torque(), states_[i].torque_B, 0.001);
    TestTwoVectors("servo current", b[i].ServoCurrent(), states_[i].servo_current, 0.001);  // working
    TestTwoFloats("current", b[i].ImpellerCurrent(), states_[i].impeller_current, 0.001);  // working
    TestTwoVectors("theta", b[i].NozzlesTheta(), states_[i].nozzle_theta, 0.001);  // working
    TestTwoFloats("motor speed", b[i].MotorSpeed(), states_[i].motor_speed, 0.001);  // working
  }
}
