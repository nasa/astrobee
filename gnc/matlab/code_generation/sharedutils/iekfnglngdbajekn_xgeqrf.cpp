//
// File: iekfnglngdbajekn_xgeqrf.cpp
//
...
void iekfnglngdbajekn_xgeqrf(real32_T A_data[], int32_T A_sizes[2], real32_T
  ...
  *tau_sizes = (int32_T)(int8_T)mn;
  if (A_sizes[0] != 0) {
    ...
    for (i = 0; (int32_T)(i + 1) <= mn; i++) {
      ...
      if ((int32_T)(i + 1) < m) {
        b_atmp = A_data[i_i];
        b = phdjbaaiglngbaaa_xzlarfg(mmi, &b_atmp, A_data, (int32_T)(i_i + 2));
        tau_data[i] = b;
        A_data[i_i] = b_atmp;
      } else {
        tau_data[i] = 0.0F;
      ...
      if ((int32_T)(i + 1) < 6) {
        b_atmp = A_data[i_i];
        A_data[i_i] = 1.0F;
        mgdjjekffknghlfk_xzlarf(mmi, (int32_T)(5 - i), (int32_T)(i_i + 1),
