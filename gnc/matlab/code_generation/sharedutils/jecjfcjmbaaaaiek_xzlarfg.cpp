//
// File: jecjfcjmbaaaaiek_xzlarfg.cpp
//
...
real32_T jecjfcjmbaaaaiek_xzlarfg(int32_T n, real32_T *alpha1, real32_T x_data[],
  int32_T ix0)
{
  real32_T tau;
  real32_T xnorm;
  ...
  tau = 0.0F;
  if (!(n <= 0)) {
    xnorm = kfknaaaamglfppph_xnrm2((int32_T)(n - 1), x_data, ix0);
    if (xnorm != 0.0F) {
      ...
      if ((real32_T)fabs((real_T)xnorm) < 9.86076132E-32F) {
        ...
        xnorm = rt_hypotf_snf(*alpha1, kfknaaaamglfppph_xnrm2((int32_T)(n - 1),
          x_data, ix0));
        if (*alpha1 >= 0.0F) {
          xnorm = -xnorm;
        }
