...
EkfWrapper::EkfWrapper(cmc_msg * cmc) :
  ekf_(cmc), ekf_initialized_(false), imus_dropped_(0), have_imu_(false),
  input_mode_(ff_msgs::SetEkfInputRequest::MODE_NONE),
  estimating_bias_(false), disp_features_(false) {}

EkfWrapper::~EkfWrapper() {
}

void EkfWrapper::Initialize(ros::NodeHandle* nh, std::string const& platform_name) {
...
void EkfWrapper::ReadParams(config_reader::ConfigReader* config) {
  if (!config->GetInt("bias_required_observations", &bias_required_observations_))
...
int EkfWrapper::Step() {
  // don't modify anything while we're copying the data
  {
    ...
    std::lock(mutex_act_msg_, mutex_vl_msg_, mutex_of_msg_);
