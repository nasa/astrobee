...
void Ekf::UpdateState(ff_msgs::EkfState* state) {
  ...
  state->ml_aug_pose.position    = msg_conversions::array_to_ros_point(gnc_.kfl_.ml_P_cam_ISS_ISS);
  state->ml_aug_pose.orientation = msg_conversions::array_to_ros_quat(gnc_.kfl_.ml_quat_ISS2cam);
  ...
  for (unsigned int i = 0; i < of_history_size_; i++) {
    state->of_aug_poses[i].position    = msg_conversions::array_to_ros_point(&gnc_.kfl_.of_quat_ISS2cam[4 * i]);
    state->of_aug_poses[i].orientation = msg_conversions::array_to_ros_quat(&gnc_.kfl_.of_P_cam_ISS_ISS[3 * i]);
  }
}
...
void Ekf::ResetPose(const Eigen::Affine3d & camera_to_body, geometry_msgs::Pose const& pose) {
  ...
  Eigen::Quaterniond world_q_body(pose.orientation.w, pose.orientation.x,
                                  pose.orientation.y, pose.orientation.z);
  Eigen::Quaterniond camera_to_body_rotation(camera_to_body.linear());
  Eigen::Vector3d world_r_body(pose.position.x, pose.position.y, pose.position.z);
  world_q_body = world_q_body * camera_to_body_rotation.conjugate();
  Eigen::Quaterniond q1(0, camera_to_body.translation().x(), camera_to_body.translation().y(),
                        camera_to_body.translation().z());
  ...
  gnc_.Initialize();
  memcpy(&vis_,  &gnc_.vis_,  sizeof(cvs_landmark_msg));
  memcpy(&reg_,  &gnc_.reg_,  sizeof(cvs_registration_pulse));
  memcpy(&of_,   &gnc_.of_,   sizeof(cvs_optical_flow_msg));
  memcpy(&hand_, &gnc_.hand_, sizeof(cvs_handrail_msg));
  memcpy(&imu_,  &gnc_.imu_,  sizeof(imu_msg));
