...
#ifndef GNC_ROS_WRAPPER_EKF_WRAPPER_H_
#define GNC_ROS_WRAPPER_EKF_WRAPPER_H_
...
class EkfWrapper {
 public:
  ...
  explicit EkfWrapper(cmc_msg * cmc);
  ~EkfWrapper();

  void Initialize(ros::NodeHandle* nh, std::string const& name);
  ...
  kfl_msg* GetOutput(void) {
    return ekf_.GetOutput();
  }
 ...
 protected:
  ...
  ros::NodeHandle* nh_;
  // topic subscribers
  ros::Subscriber imu_sub_, of_sub_, vl_sub_, ar_sub_, dl_sub_, truth_sub_;
...
#endif  // GNC_ROS_WRAPPER_EKF_WRAPPER_H_
