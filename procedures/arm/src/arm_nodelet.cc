...
class ArmNodelet : public ff_util::FreeFlyerNodelet {
 public:
  // Possible events
  enum ArmEvent {
    READY,                   // We are connected to the arm
    DEPLOYED,                // Background deploy
    STOWED,                  // Background stow
    GOAL_DEPLOY,             // Start a new deploy action
    GOAL_STOW,               // Start a new deploy action
    GOAL_MOVE,               // Start a new move (pan and tilt)
    GOAL_PAN,                // Start a new pan action
    GOAL_TILT,               // Start a new tilt action
    GOAL_CALIBRATE,          // Start a new gripper action
    GOAL_SET,                // Start a new gripper action
    GOAL_CANCEL,             // Cancel the current goal
    PAN_COMPLETE,            // Pan complete
    TILT_COMPLETE,           // Tilt complete
    GRIPPER_COMPLETE,        // Gripper action complete
    CALIBRATE_COMPLETE,      // Calibration complete
    TIMEOUT                  // Current action doesn't complete in time
  };
  ...
  ArmNodelet() : ff_util::FreeFlyerNodelet(NODE_ARM, true),
    ...
    fsm_.Add(READY,
      STATE::INITIALIZING,
      [this](ArmEvent const& event) -> int32_t {
    ...
    //   [label="[6]\nPAN_COMPLETE\nTilt(DEPLOY)"];
    fsm_.Add(PAN_COMPLETE,
      STATE::DEPLOYING_PANNING,
      [this](ArmEvent const& event) -> int32_t {
        if (!Arm(TILT))
    ...
    fsm_.Add(TILT_COMPLETE,
      STATE::DEPLOYING_TILTING,
      [this](ArmEvent const& event) -> int32_t {
    ...
    fsm_.Add(GRIPPER_COMPLETE,
      STATE::STOWING_SETTING,
      [this](ArmEvent const& event) -> int32_t {
        if (!Arm(PAN))
          return Result(RESPONSE::PAN_FAILED);
        return STATE::STOWING_PANNING;
 ...
 protected:
  ...
  void Initialize(ros::NodeHandle *nh) {
    ...
    cfg_.Initialize(GetPrivateHandle(), "proc/arm.config");
    ...
    joints_[GRIPPER].scale = 1.0;
    joints_[GRIPPER].offset = 0.0;
    ...
    server_.Create(nh, ACTION_PROCEDURES_ARM);
  }
  ...
  void UpdateCallback(int32_t state, ArmEvent event) {
    // Debug events
    ...
    case GOAL_PAN:                 str = "GOAL_PAN";            break;
    case GOAL_TILT:                str = "GOAL_TILT";           break;
    ...
    if (joints_[GRIPPER].val  < 0)
    ...
    else if (fabs(joints_[GRIPPER].val = K_GRIPPER_OPEN) < tol)
      ...
      msg.gripper_state.state = ff_msgs::ArmGripperState::UNKNOWN;
  ...
  bool Equal(JointType t, double v) {
    ...
    return (fabs(joints_[t].val - v) < joints_[t].tol);
  }
  ...
  void JointStateCallback(sensor_msgs::JointState::ConstPtr const& msg) {
    ...
    for (size_t i = 0; i < msg->name.size(); i++) {
      ...
      // Set the value in a generic way
      joints_[generic].val =
        msg->position[i] * joints_[generic].scale + joints_[generic].offset;
      // Assemble the joint sample. Right now I believe that executive
      // is expecting this in SI units. Eventually, we'll probably want to
      // output this in HUMAN units.
      static ff_msgs::JointSample js;
      js.name = joints_[generic].generic;
      js.angle_vel = msg->position[i];
      js.angle_pos = msg->velocity[i];
      js.current = msg->effort[i];
  ...
  void GoalCallback(ff_msgs::ArmGoalConstPtr const& goal) {
    ...
    case ff_msgs::ArmGoal::ARM_PAN:
      NODELET_DEBUG_STREAM("Received a new ARM_PAN command");
      if (fsm_.GetState() == STATE::DEPLOYED) {
        // Check that the value is reasonable
        if (goal->pan < K_PAN_MIN || goal->pan > K_PAN_MAX) {
          Result(RESPONSE::BAD_PAN_VALUE, true);
          return;
        }
        // Set the goal
        joints_[PAN].goal = goal->pan;
        // Start the action
    ...
    case ff_msgs::ArmGoal::ARM_TILT:
      NODELET_DEBUG_STREAM("Received a new ARM_TILT command");
      if (fsm_.GetState() == STATE::DEPLOYED) {
        // Check that the value is reasonable
        if (goal->tilt < K_TILT_MIN || goal->tilt > K_TILT_MAX) {
          Result(RESPONSE::BAD_TILT_VALUE, true);
          return;
        }
        // Set the goal
        joints_[TILT].goal = goal->tilt;
        // Start the action
        return fsm_.Update(GOAL_MOVE);
      }
      break;
    ...
    case ff_msgs::ArmGoal::ARM_MOVE:
      NODELET_DEBUG_STREAM("Received a new ARM_MOVE command");
      if (fsm_.GetState() == STATE::DEPLOYED) {
        // Check that the pan value is reasonable
        if (goal->pan < K_PAN_MIN || goal->pan > K_PAN_MAX) {
          Result(RESPONSE::BAD_PAN_VALUE, true);
          return;
        }
        // Check that the tilt value is reasonable
        if (goal->tilt < K_TILT_MIN || goal->tilt > K_TILT_MAX) {
          Result(RESPONSE::BAD_TILT_VALUE, true);
          return;
        }
 ...
 protected:
  ...
  ff_util::FiniteStateMachine<int32_t, ArmEvent> fsm_;
  // Config server
  ...
  static constexpr double K_TILT_MIN        =  -30.0;
  static constexpr double K_TILT_MAX        =   90.0;
  static constexpr double K_TILT_STOW       =  180.0;
  static constexpr double K_TILT_DEPLOY     =    0.0;
  static constexpr double K_GRIPPER_OFFSET  =    0.0;
