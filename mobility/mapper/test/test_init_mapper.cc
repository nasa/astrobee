...
TEST(init_mapper, MapperInitFailed) {
  ...
  if (!fault_config.CheckValExists("mapper")) {
    ROS_ERROR("Couldn't extract mapper faults. Needed for init fault id.");
    // Cause test to fail
    EXPECT_EQ(2, 1);
    return;
  }

  config_reader::ConfigReader::Table fault_table(&fault_config, "mapper");
  ...
  for (int i = 1; i < (fault_table.GetSize() + 1); i++) {
    ...
    if (!fault_entry.GetStr("key", &fault_key)) {
      ROS_ERROR("Fault key at index %i not in mappper fault table.", i);
      // Cause test to fail
      EXPECT_EQ(2, 1);
      return;
    }
    ...
    if (fault_key == "INITIALIZATION_FAILED") {
      if (!fault_entry.GetInt("id", &init_fault_id)) {
        ROS_ERROR("Fault id for mapper init failed fault missing.");
        // Cause test to fail
        EXPECT_EQ(2, 1);
        return;
      }
  ...
  if (init_fault_id == -1) {
    ROS_ERROR("Unable to extract init fault id for mapper.");
    // Cause test to fail
    EXPECT_EQ(2, 1);
    return;
  }
  ...
  // Start timer to check if the mapper fault has been thrown.
