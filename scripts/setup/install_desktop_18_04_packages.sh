...
then
  ...
  if [ "${NO_TUNNEL}" -eq 1 ]; then
      ...
      sudo /bin/bash -c "echo \"deb [arch=amd64] http://astrobee.ndc.nasa.gov/software bionic main\" > $arssrc" || exit 1
      sudo /bin/bash -c "echo \"deb-src http://astrobee.ndc.nasa.gov/software bionic main\" >> $arssrc" || exit 1
  else
      echo "Tunnelling to get the custom Debian"
      sudo /bin/bash -c "echo \"deb [arch=amd64] http://127.0.0.1:8765/software bionic main\" > $arssrc" || exit 1
      sudo /bin/bash -c "echo \"deb-src http://127.0.0.1:8765/software bionic main\" >> $arssrc" || exit 1
