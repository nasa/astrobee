...
class Activity_DBGenerator:
   ...
   def callback(self, data):
      ...
      if count == 0:
         ...
         robot_pose = self.body_frame_from_navcam(data.pose.position.x, data.pose.position.y, data.pose.position.z,
   ...
   def open_file(self, file, activity, map, date):
      ...
      self.fileOut.write("### Date-Map-Activity: " + date + "-" + map + "-" + activity +"\n")
...
if __name__ == '__main__':
   ...
   try:
       ...
       if len(sys.argv) < 5:
          print("Usage: activity_db_generator.py <activity date> <map name> <activity name> <location to save>")
          ...
          output_filename = location_name + activity_date + "_" + map_name + "_" + activity_name + "_db.csv"
