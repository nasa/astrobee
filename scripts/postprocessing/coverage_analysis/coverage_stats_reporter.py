...
class Coverage_StatsReporter:
        ...
        with open(fileIn) as f_in:
            for line in f_in:
                ...
                if re.match("^\s*\#", line):
                    ...
                    if vals[1] == "Wall":
                        if wall_counter == -1:
                            self.coverage_per_wall[0] = vals[
                                2
                            ]  # Save the name of the wall
                            wall_counter += 1

                        else:
                            # Save the coverage of this wall in final array
                            self.final_stats.insert(
                                wall_counter, self.coverage_per_wall
                            )
    ...
    def write_report(self, fileIn):
        ...
        with open(stats_filename, "w") as f_stats:
            f_stats.write(
                "# Coverage Distribution along the different walls of the JEM\n"
            )
            f_stats.write(
                "# Columns represent percentage of registered ML Features per wall (0, 1-10, 11-20, 21-40, 40+)\n"
            )
            f_stats.write("# Analyisis corresponding to: {:s}\n".format(fileIn))
            f_stats.write(
                "JEM Wall, 0ML, 1-10ML, 11-20ML, 21-40ML, 40+ML, Total Reg. ML Features per wall, Percentage of Total Num ML Features Analyzed\n"
            )
            ...
            for i in range(len(self.final_stats)):
                self.final_stats[i].extend(
                    [self.final_stats[i][last_column] * 100.0 / max_num_features]
                )
            for i in range(len(self.final_stats)):
                f_stats.write(
                    "{:s}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}\n".format(
                        ...
                        self.final_stats[i][7],
                    )
                )  # Total Num ML Features Analyzed %
            f_stats.write(
                "Total Number of Registered Features, {:d}\n".format(
    ...
    def print_pdf_report(self, fileIn):
        ...
        with PdfPages(report_filename) as pdf:
            ...
            plt.bar(
                ...
                label="21-40ML",
                color="green",
                bottom=third_bin + second_bin + first_bin,
            )
            plt.bar(
                x_data,
                fifth_bin,
                ...
                label="40+ML",
                color="blue",
                bottom=fourth_bin + third_bin + second_bin + first_bin,
            )
            ...
            plt.tight_layout(
                rect=[0, 0, 0.95, 1.0]
            )  # adjusts the bottom margin of the plot so that the labels don't go off the bottom edge.
            plt.legend(
                fontsize=5, bbox_to_anchor=(0.5, -0.1), loc="upper center", ncol=5
