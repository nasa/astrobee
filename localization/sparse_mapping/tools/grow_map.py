...
import sys, os, re, subprocess, argparse, random, shutil
...
def parse_args():
    ...
    parser.add_argument("-small_map", type=str, required = True,
                        help="Input registered, pruned, BRISK map with vocab db.")
    parser.add_argument("-big_map", type=str, required = True,
...
def grow_map(args, curr_map_images, curr_map, source_map):
    ...
    bad_images = find_hard_to_localize_images(args, curr_map, source_map,
                                                  log_file)
...
def main():
    ...
    if args.small_map == args.big_map or args.small_map == args.output_map or \
       args.big_map == args.output_map:
    ...
    for list_iter in range(len(args.image_lists)):
        ...
        if len(missing_images) > 0:
            print("The following images are in " + args.big_map + " but not in " + curr_map)
        ...
        [curr_map_images, curr_map] = grow_map(args, curr_map_images, curr_map, source_map)

    # One last verification. Images in args.big_map that cannot be localized
    # against the map grown so far must be added to it.
    [curr_map_images, curr_map] = grow_map(args, curr_map_images, curr_map, args.big_map)

    if curr_map == args.small_map:
        print("The map did not grow. Create " + args.output_map + " as a copy of " + args.small_map)
