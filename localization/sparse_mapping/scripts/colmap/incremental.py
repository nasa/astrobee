...
class IncrementalMapper:
    ...
    def build_map(self, image_list, output_path):
        ...
        try:
            ...
            sys.exit(1)
        with colmap.ColmapProjectConfig(self.database_path, self.image_path, output_path, image_list, input_path=output_path) as cfg:
            os.system('colmap mapper --project_path %s > %s/colmap.out 2>&1' % (cfg.file_name(), output_path))
    ...
    def merge_models(self, model1, model2):
        ...
        with colmap.ColmapProjectConfig(self.database_path, self.image_path, model1.filename, images, ini_file='merge.ini', input_path=model1.filename) as cfg:
            os.system('colmap mapper --project_path %s >> %s/colmap_add.out 2>&1' % (cfg.file_name(), model1.filename))
        print('Results: %s' % (colmap.Model(model1.filename)))
        os.mkdir(model1.filename + '_merged')
        os.system('colmap model_merger --input_path1 %s --input_path2 %s --output_path %s' % (model2.filename, model1.filename, model1.filename + '_merged'))
        print('Merged maps: %s' % (colmap.Model(model1.filename + '_merged')))
        os.mkdir(model1.filename + '_points')
        with colmap.ColmapProjectConfig(self.database_path, self.image_path, model1.filename + '_points', images, ini_file='merge.ini', input_path=model1.filename + '_merged') as cfg:
            os.system('colmap mapper --project_path %s >> %s/colmap_points.out 2>&1' % (cfg.file_name(), model1.filename))
        ...
        os.system('colmap bundle_adjuster --input_path %s --output_path %s >> %s/colmap_ba.out 2>&1' % (model1.filename + '_points', model1.filename + '_ba', model1.filename))
    ...
    def merge_models_recursive(self, images, subdir=''):
        ...
        model = models.pop(0)
        print('Starting with %s: %s' % (model.filename[len(self.output_path) + 5:], model))
        ...
        while len(models) > 0:
            ...
            print('Merging %s with %d shared features: %s' % (to_merge.filename[len(self.output_path) + 5:], shared_features, to_merge))
    ...
    def incremental_add(self):
        ...
        print('Building initial map %s from %s with %d images...' % (str(map_num), data_source, len(images)))
        ...
        while len(subdirs) > 0:
            ...
            print('Building map %s, adding %s with %d images and %d overlapping features.' % (str(map_num), data_source, len(new_images), best_matches))
            os.mkdir(next_map_path)
            with colmap.ColmapProjectConfig(self.database_path, self.image_path, next_map_path, images, ini_file='merge.ini', input_path=map_path) as cfg:
                os.system('colmap mapper --project_path %s > %s/colmap.out 2>&1' % (cfg.file_name(), next_map_path))
...
def main():
    parser = argparse.ArgumentParser(description="Incrementally build a map with colmap from a nested directory of images.")
    parser.add_argument("--database_path", required=True, help="Colmap database file.")
    parser.add_argument("--output_path", required=True, help="Path to write output model.")
