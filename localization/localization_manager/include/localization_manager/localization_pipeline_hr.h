...
#ifndef LOCALIZATION_MANAGER_LOCALIZATION_PIPELINE_HR_H_
#define LOCALIZATION_MANAGER_LOCALIZATION_PIPELINE_HR_H_
...
class HRPipeline : public Pipeline {
 ...
 protected:
  ...
  // Callback resets watchdog timer and performs some extra sanity checks
  void FeatureCallback(ff_msgs::DepthLandmarks::ConstPtr const& msg);

  // Let the localization manager know that we didn't have enough valid measurements
  void MeasurementWatchdog(ros::TimerEvent const& event);

  // Let the localization manager know that we didn't have enough features
  void FeatureWatchdog(ros::TimerEvent const& event);

  // Called back when the stability timer registers
  void StableCallback(ros::TimerEvent const& event);

  // Called back when the timeout timer registers
...
#endif  // LOCALIZATION_MANAGER_LOCALIZATION_PIPELINE_HR_H_
