...
MLPipeline::MLPipeline(ros::NodeHandle *nh, ros::NodeHandle *nhp, uint8_t mode, PipelineCallbackType cb)
  ...
  cfg_.Initialize(&nh_pvt_, "localization/localization_pipeline_ml.config");
  cfg_.Listen(boost::bind(&MLPipeline::ReconfigureCallback, this, _1));
  // Create the timers
  timer_m_ = nh->createTimer(ros::Duration(cfg_.Get<double>("timeout_measurements")),
    &MLPipeline::MeasurementWatchdog, this, false, false);
  timer_f_ = nh->createTimer(ros::Duration(cfg_.Get<double>("timeout_features")),
    &MLPipeline::FeatureWatchdog, this, false, false);
  ...
  timer_stable_ = nh->createTimer(ros::Duration(cfg_.Get<double>("timeout_stability")),
    &MLPipeline::StableCallback, this, true, false);
  // Enable or disable the handrail detector
  service_.SetConnectedTimeout(cfg_.Get<double>("timeout_service_enable"));
  service_.SetTimeoutCallback(std::bind(&MLPipeline::EnableTimeoutCallback, this));
  service_.Create(nh, SERVICE_LOCALIZATION_ML_ENABLE);
}

bool MLPipeline::ReconfigureCallback(dynamic_reconfigure::Config &config) {
...
void MLPipeline::EnableTimeoutCallback() {
  ROS_ERROR("SERVICE_LOCALIZATION_ML_ENABLE client timeout");
}

bool MLPipeline::Enable(bool enable) {
  ...
  if (enable) {
    ...
    sub_m_ = nh_->subscribe(TOPIC_LOCALIZATION_ML_REGISTRATION,
      1, &MLPipeline::MeasurementCallback, this);
    sub_f_ = nh_->subscribe(TOPIC_LOCALIZATION_ML_FEATURES,
      1, &MLPipeline::FeatureCallback, this);
...
void MLPipeline::MeasurementCallback(ff_msgs::CameraRegistration::ConstPtr const& msg) {
  StartTimer(timer_m_, cfg_.Get<double>("timeout_measurements"));
}

void MLPipeline::FeatureCallback(ff_msgs::VisualLandmarks::ConstPtr const& msg) {
  if (static_cast<int>(msg->landmarks.size()) >= cfg_.Get<int>("minimum_features"))
    StartTimer(timer_f_, cfg_.Get<double>("timeout_features"));
}

void MLPipeline::MeasurementWatchdog(ros::TimerEvent const& event) {
  MarkStable(false);
  StartTimer(timer_stable_, cfg_.Get<double>("timeout_stability"));
}

void MLPipeline::FeatureWatchdog(ros::TimerEvent const& event) {
  MarkStable(false);
  StartTimer(timer_stable_, cfg_.Get<double>("timeout_stability"));
}

void MLPipeline::StableCallback(ros::TimerEvent const& event) {
  MarkStable(true);
  StartTimer(timer_stable_, cfg_.Get<double>("timeout_stability"));
}
