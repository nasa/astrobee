...
GraphLocalizerWrapper::GraphLocalizerWrapper(const std::string& graph_config_path_prefix)
  ...
  graph_vio_initializer_.LoadGraphLocalizerParams(config);
...
bool GraphLocalizerWrapper::Initialized() const { return (graph_vio_.get() != nullptr); }
...
void GraphLocalizerWrapper::InitializeGraph() {
  ...
  graph_vio_.reset(new graph_vio::GraphLocalizer(graph_vio_initializer_.params()));
}
...
boost::optional<const GraphLocalizer&> GraphLocalizerWrapper::graph_vio() const {
  if (!graph_vio_) return boost::none;
  return *graph_vio_;
}
...
boost::optional<const GraphLocalizerStats&> GraphLocalizerWrapper::graph_vio_stats() const {
  if (!graph_vio_) {
