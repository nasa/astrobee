...
TEST(UtilitiesTester, InterpolatePosesWithCovariance) {
  const auto t1 = lc::RandomVector3d();
  const auto t2 = lc::RandomVector3d();
  const auto rot1 = lc::RotationFromEulerAngles(0, 0, 0);
  const auto rot2 = lc::RotationFromEulerAngles(180, 0, 0);
  const auto covariance = lc::PoseCovariance::Identity();
  const auto pose1 = lc::PoseWithCovariance(lc::Isometry3d(t1, rot1), covariance);
  const auto pose2 = lc::PoseWithCovariance(lc::Isometry3d(t2, rot2), covariance);

  // 0 alpha
  {
    const auto interpolated_pose = lc::Interpolate(pose1, pose2, 0);
    EXPECT_MATRIX_NEAR(interpolated_pose.pose, pose1.pose, 1e-6);
  }

  // 1 alpha
  {
    const auto interpolated_pose = lc::Interpolate(pose1, pose2, 1);
    EXPECT_MATRIX_NEAR(interpolated_pose.pose, pose2.pose, 1e-6);
  }
  ...
  // 0.5 alpha
  {
    ...
    const auto expected_rotation = lc::RotationFromEulerAngles(90, 0, 0);
    const auto expected_pose = lc::Isometry3d(expected_translation, expected_rotation);
    EXPECT_MATRIX_NEAR(interpolated_pose.pose, expected_pose, 1e-6);
  }
