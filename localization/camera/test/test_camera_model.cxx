...
TEST(camera_model, angleaxis_transforms) {
  ...
  Eigen::Matrix3d R1 = Eigen::AngleAxisd(-.156 * M_PI, Eigen::Vector3d(.14, -.56, .4).normalized()).matrix();
  Eigen::Matrix3d R2;
  ...
  ceres::AngleAxisRotatePoint(&rodrigues[0], &input[0], &output1[0]);
  for (int i = 0; i < 3; i++) EXPECT_NEAR(output1[i], output2[i], 1e-6);
}
...
TEST(camera_model, camera) {
  ...
  EXPECT_VECTOR2D_NEAR(camera.GetParameters().GetUndistortedSize(), camera2.GetParameters().GetUndistortedSize(), 0);
...
TEST(camera_model, DistortionCoeffs) {
  ...
  std::vector<std::string> files{"p2_camera.xml", "iss_tango_camera.xml", "earth_tango_camera.xml",
                                 "iss_tango_undistorted.xml", "earth_tango_undistorted.xml"};
  for (std::string const& file : files) {
    ...
    for (float x = -320; x < 320; x += 30) {
      for (float y = -240; y < 240; y += 30) {
        ...
        camera_params.Convert<camera::UNDISTORTED_C, camera::DISTORTED_C>(Eigen::Vector2d(x, y), &distort_location);
        camera_params.Convert<camera::DISTORTED_C, camera::UNDISTORTED_C>(distort_location, &loopback_location);
