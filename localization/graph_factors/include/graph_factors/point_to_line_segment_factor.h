...
#ifndef GRAPH_FACTORS_POINT_TO_LINE_SEGMENT_FACTOR_H_
#define GRAPH_FACTORS_POINT_TO_LINE_SEGMENT_FACTOR_H_

#include <graph_factors/point_to_line_factor_base.h>
 ...
 public:
  ...
  Vector evaluateError(const Pose3& world_T_body, boost::optional<Matrix&> H = boost::none) const override {
    ...
    if (!use_silu_) {
      ...
      if (H) {
        Matrix11 d_silu_d_z;
        error.z() = graph_factors::SiluWithOffsetTwoWay(error.z(), line_length_ / 2.0, d_silu_d_z);
        H->block<1, 6>(2, 0) = d_silu_d_z * H->block<1, 6>(2, 0);
      }
      error.z() = graph_factors::SiluWithOffsetTwoWay(error.z(), line_length_ / 2.0);
    }
    return error;
  }
...
#endif  // GRAPH_FACTORS_POINT_TO_LINE_SEGMENT_FACTOR_H_
