...
TEST(SiluTester, Jacobian) {
  for (int i = 0; i < 500; ++i) {
    ...
    const auto numerical_H = gtsam::numericalDerivative11<double, double>(
      boost::function<double(const double)>(boost::bind(&gf::Silu, _1, boost::none)), x);
...
TEST(SiluWithOffsetTester, Jacobian) {
  for (int i = 0; i < 500; ++i) {
    ...
    gtsam::Matrix H;
    gf::SiluWithOffset(x, offset, H);
    const auto numerical_H = gtsam::numericalDerivative21<double, double, double>(
      boost::function<double(const double, const double)>(boost::bind(&gf::SiluWithOffset, _1, _2, boost::none)), x,
      offset);
    ASSERT_TRUE(gtsam::equal_with_abs_tol(numerical_H, H.matrix(), 1e-6));
  }
}
...
TEST(SiluWithOffsetTester, XMuchGreaterThanOffset) {
  ...
  const double silu_x = gf::SiluWithOffset(x, offset);
...
TEST(SiluWithOffsetTwoWayTester, Jacobian) {
  for (int i = 0; i < 500; ++i) {
    ...
    const double offset = lc::RandomPositiveDouble();
    gtsam::Matrix H;
    gf::SiluWithOffsetTwoWay(x, offset, H);
    const auto numerical_H = gtsam::numericalDerivative21<double, double, double>(
      boost::function<double(const double, const double)>(boost::bind(&gf::SiluWithOffsetTwoWay, _1, _2, boost::none)),
      x, offset);
    ASSERT_TRUE(gtsam::equal_with_abs_tol(numerical_H, H.matrix(), 1e-6));
  }
}
...
TEST(SiluWithOffsetTwoWayTester, XMuchGreaterThanOffset) {
  const double x = 3001.3;
  const double offset = 5.1;
  const double silu_x = gf::SiluWithOffsetTwoWay(x, offset);
  EXPECT_NEAR(x - offset, silu_x, 1e-6);
}
