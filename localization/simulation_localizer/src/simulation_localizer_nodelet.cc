...
#include <simulation_localizer/simulation_localizer_nodelet.h>
...
void SimulationLocalizerNodelet::SubscribeAndAdvertise(ros::NodeHandle* nh) {
  ...
  pose_sub_ = nh->subscribe(TOPIC_LOCALIZATION_TRUTH, 1, &SimulationLocalizerNodelet::PoseCallback, this,
                            ros::TransportHints().tcpNoDelay());
  twist_sub_ = nh->subscribe(TOPIC_LOCALIZATION_TRUTH_TWIST, 1, &SimulationLocalizerNodelet::TwistCallback, this,
                             ros::TransportHints().tcpNoDelay());

  input_mode_srv_ = nh->advertiseService(SERVICE_GNC_EKF_SET_INPUT, &SimulationLocalizerNodelet::SetMode, this);
}

bool SimulationLocalizerNodelet::SetMode(ff_msgs::SetEkfInput::Request& req, ff_msgs::SetEkfInput::Response& res) {
  input_mode_ = req.mode;
  return true;
}

void SimulationLocalizerNodelet::PoseCallback(geometry_msgs::PoseStamped::ConstPtr const& pose) {
...
void SimulationLocalizerNodelet::TwistCallback(geometry_msgs::TwistStamped::ConstPtr const& twist) {
...
void SimulationLocalizerNodelet::PublishLocState(const lc::Time& timestamp) {
