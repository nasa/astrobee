...
ImuIntegrator::ImuIntegrator(const Eigen::Isometry3d& body_T_imu, const Eigen::Vector3d& gyro_bias,
                             const Eigen::Vector3d& accelerometer_bias, const Time start_time,
  ...
  // Set sensor covariances TODO(rsoussan): change these when receive new measurements? also set these in factor?
...
void ImuIntegrator::BufferImuMeasurement(const ImuMeasurement& imu_measurement) {
  if (imu_measurement.timestamp < start_time_) {
    LOG(WARNING)
        << "BufferImuMeasurement: Failed to add imu measurement since timestamp is before start time, ignoring "
           "measurement.";
    return;
  }
...
void ImuIntegrator::IntegrateLatestImuMeasurements(const Time end_time) {
  if (measurements_.size() < 2) {
    LOG(FATAL) << "IntegrateLatestImuMeasurements: Less than 2 measurements available.";
  }
...
Time ImuIntegrator::IntegrateImuMeasurements(const Time start_time, const Time end_time,
  ...
  Time last_added_imu_measurement_time = start_time;
  ...
  DLOG(INFO) << "IntegrateImuMeasurements: Total Num Imu Measurements after integrating: " << measurements_.size();
  return last_added_imu_measurement_time;
}

void ImuIntegrator::RemoveOldMeasurements(const Time new_start_time) {
...
boost::shared_ptr<gtsam::PreintegratedCombinedMeasurements::Params> ImuIntegrator::pim_params() const {
  // Make a copy so internal params aren't exposed.  Gtsam expects a point to a non const type, so can't pass a pointer
  // to const.
...
Time ImuIntegrator::OldestTime() const {
  if (Empty()) {
...
Time ImuIntegrator::LatestTime() const {
  if (Empty()) {
