...
#ifndef GRAPH_LOCALIZER_TOLERANT_PROJECTION_FACTOR_H_
#define GRAPH_LOCALIZER_TOLERANT_PROJECTION_FACTOR_H_
...
class TolerantProjectionFactor : public NoiseModelFactor1<POSE> {
 protected:
 ...
 public:
  ...
  typedef TolerantProjectionFactor<POSE, LANDMARK, CALIBRATION> This;
  ...
  /** Virtual destructor */
  virtual ~TolerantProjectionFactor() {}
  ...
  void print(const std::string& s = "", const KeyFormatter& keyFormatter = DefaultKeyFormatter) const {
    std::cout << s << "TolerantProjectionFactor, z = ";
  ...
  bool cheiralityError(const Pose3& pose) const {
    try {
      if (body_P_sensor_) {
        PinholeCamera<CALIBRATION> camera(pose.compose(*body_P_sensor_), *K_);
        camera.project(landmark_point_) - measured_;
      } else {
        PinholeCamera<CALIBRATION> camera(pose, *K_);
        camera.project(landmark_point_) - measured_;
      }
...
struct traits<TolerantProjectionFactor<POSE, LANDMARK, CALIBRATION>>
    : public Testable<TolerantProjectionFactor<POSE, LANDMARK, CALIBRATION>> {};

}  // namespace gtsam

#endif  // GRAPH_LOCALIZER_TOLERANT_PROJECTION_FACTOR_H_
