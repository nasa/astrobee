...
def run_ekf(
    ...
    cmd = (
        "bag_to_csv"
        if ekf_in_bag
        else "bag_to_csv --run_ekf %s" % ("" if features_in_bag else "--gen_features")
    )

    robot_bias = os.path.dirname(os.path.abspath(astrobee_bag)) + "/imu_bias.config"
    cmd = "rosrun ekf_bag %s %s %s %s %s" % (
        cmd,
        astrobee_map,
        astrobee_bag,
        output_file,
    ...
    if not ekf_in_bag:
        print(
            (
                "Using features from bag."
                if features_in_bag
                else "Generating features from image."
            )
        )
...
def prune_missing_timestamps(a_xs, a_ys, a_zs, a_times, b_times):
    ...
    for b_index, b_time in enumerate(b_times):
        while (
            not np.isclose(a_times[a_index], b_time, rtol=0)
            and a_times[a_index] < b_time
            and a_index < len(a_times)
        ):
            a_index += 1
        ...
        pruned_a_matrix[b_index] = np.array(
            [a_xs[a_index], a_ys[a_index], a_zs[a_index]]
        )
...
def orientation_rmse(a_xs, a_ys, a_zs, b_xs, b_ys, b_zs, a_times, b_times):
    ...
    for b_index, b_time in enumerate(b_times):
        ...
        if not np.isclose(a_times[a_index], b_time, rtol=0, atol=0.02):
            print("Failed to find a time close to b time.")
            continue
        a_rot = scipy.spatial.transform.Rotation.from_euler(
            "ZYX", [a_xs[a_index], a_ys[a_index], a_zs[a_index]], degrees=False
        )
        b_rot = scipy.spatial.transform.Rotation.from_euler(
            "ZYX", [b_xs[b_index], b_ys[b_index], b_zs[b_index]], degrees=False
        )
        rot_diff = a_rot.inv() * b_rot
        rot_squared_error = np.inner(rot_diff.as_rotvec(), rot_diff.as_rotvec())
        mean_squared_orientation_error += (
            rot_squared_error - mean_squared_orientation_error
        ) / (b_index + 1)
...
def rmse_timestamped_sequences(a_xs, a_ys, a_zs, a_times, b_xs, b_ys, b_zs, b_times):
    a_positions, missing_indices = prune_missing_timestamps(
        a_xs, a_ys, a_zs, a_times, b_times
    )
...
def covariance_map(ekf, cov_1_index, cov_2_index, cov_3_index):
    return [
        math.sqrt(x_y_z[0] + x_y_z[1] + x_y_z[2])
        for x_y_z in zip(ekf[cov_1_index], ekf[cov_2_index], ekf[cov_3_index])
    ]
...
def integrated_velocities(start_x, vs, ts):
    ...
    xs = [
        start_x + cumulative_x_increment
...
class EkfLog(object):
    def __init__(self, filename, start_time=float("-inf"), end_time=float("inf")):
        ...
        for l in f:
            ...
            if l.startswith("EKF "):
                ...
                q = (
                    0.5 * self.ekf["cov_1"][-1],
                    0.5 * self.ekf["cov_2"][-1],
                    0.5 * self.ekf["cov_3"][-1],
                    1.0,
                )
                ...
                if t > 0.0:
                    ...
                    for i in range((len(p) - 3) / 4):
                        ...
                        self.of_heatmap[
                            int(round(u / self.CAMERA_RES[0] * self.HEATMAP_SIZE[0]))
                            + self.HEATMAP_SIZE[0] / 2,
                            int(round(v / self.CAMERA_RES[1] * self.HEATMAP_SIZE[1]))
                            + self.HEATMAP_SIZE[1] / 2,
                        ] += 1
    ...
    def evaluate_error(self, time_offset=0.0):
        ...
        for gt_i in range(len(self.gt["t"])):
            ...
            u = (t - self.ekf["t"][ekf_i - 1]) / (
                self.ekf["t"][ekf_i] - self.ekf["t"][ekf_i - 1]
            )
            x1 = np.array(
                [
                    self.ekf["x"][ekf_i - 1],
            ...
            x2 = np.array(
                [self.ekf["x"][ekf_i], self.ekf["y"][ekf_i], self.ekf["z"][ekf_i]]
            )
            x = x1 + u * (x2 - x1)
            # but just use the older angle, not worth trouble to interpolate in python
            a = np.array(
                [
                    self.ekf["angle1"][ekf_i - 1],
                    self.ekf["angle2"][ekf_i - 1],
                    self.ekf["angle3"][ekf_i - 1],
                ]
            )
            ...
            pos_err += (
                float(
                    norm(
                        np.array(
                            [self.gt["x"][gt_i], self.gt["y"][gt_i], self.gt["z"][gt_i]]
                        )
                        - x
                    )
                )
    ...
    def rmse_using_single_image_sparse_mapping(self):
        rmse_pos = rmse_timestamped_sequences(
            self.ekf["x"],
            self.ekf["y"],
            self.ekf["z"],
            self.ekf["t"],
        ...
        rmse_int_v = rmse_timestamped_sequences(
            int_x,
            int_y,
            int_z,
            self.ekf["t"],
    ...
    def get_stats(self, job_id):
        ...
        # OF stats
        of_count_no_nans = [
            0 if math.isnan(val) else val for val in self.ekf["of_count"]
        ]
        ...
        ratio_of_features_added = (
            num_of_features_added / num_of_features_observed
            if num_of_features_observed > 0
            else 0
        )
        ...
        # ML stats
        ml_count_no_nans = [
            0 if math.isnan(val) else val for val in self.ekf["ml_count"]
        ]
        ...
        ratio_ml_features_added = (
            num_ml_features_added / num_ml_features_observed
            if num_ml_features_observed > 0
            else 0
        )
        ...
        # Mahal
        avg_mahal_distance = (
            np.mean(np.concatenate(self.mahal["boxes"]))
        ...
        (
            rmse_pos_vs_single_image_sparse_mapping,
            rmse_angle_vs_single_image_sparse_mapping,
            rmse_int,
        ) = self.rmse_using_single_image_sparse_mapping()
    ...
    def plot(self, filename, output_text):
        ...
        with PdfPages(filename) as pdf:
            ...
            plt.plot(
                gt["t"], gt["angle2"], color=colors[1], linewidth=0.25, dashes=(1, 1)
            )
            plt.plot(
                gt["t"], gt["angle3"], color=colors[2], linewidth=0.25, dashes=(1, 1)
            )
            plt.plot(
                vl["t"],
                vl["angle1"],
            ...
            if len(self.mahal["boxes"]) > 0:
                ...
                plt.setp(
                    boxes["boxes"], color="Black", facecolor="SkyBlue", linewidth=0.25
                )
            ...
            plt.figure()
            plt.plot(
                ekf["t"], ekf["vx"], color=colors[0], linewidth=0.5, label="Velocity"
            )
            ...
            ax2.plot(
                ekf["t"], ekf["gby"], color=colors[1], linewidth=0.5, dashes=(1, 1)
            )
            ax2.plot(
                ekf["t"], ekf["gbz"], color=colors[2], linewidth=0.5, dashes=(1, 1)
            )
            ax2.fill_between(
                ekf["t"],
                ekf["gbx"] - ekf["cov_4"],
            ...
            plt.figure()
            plt.plot(
                of["t"], of["oldest"], color=colors[0], linewidth=0.5, label="Oldest"
            )
            plt.plot(
                of["t"], of["median"], color=colors[1], linewidth=0.5, label="Median"
            )
            plt.plot(
                of["t"],
                of["youngest"],
...
class RunEKFOptions(object):
    ...
    def set_param_sweep_params(
        self, image_topic, job_id, gnc_config, save_stats=True, make_plots=True
    ):
...
def run_ekf_and_save_stats(options):
    (astrobee_map, astrobee_bag, robot_config) = environment.initialize_environment(
        options.map_file, options.bag_file, options.robot_name
    )
    ...
    if not options.cached:
        run_ekf(
            astrobee_map,
            astrobee_bag,
            options.ekf_output_file,
            options.ekf_in_bag,
            options.features_in_bag,
            options.image_topic,
            options.gnc_config,
        )
    ...
    print(
        (
            "Finished running EKF for job_id "
            + str(options.job_id)
            + " in "
            + str(run_time)
            + " seconds"
    ...
    if options.save_stats:
        log.write_results_to_csv(
            options.job_id, options.results_csv_output_file, astrobee_bag
        )
    ...
    if options.make_plots:
        ...
        output_text += "VL Feature Throughput: %g VLs / s Max Gap: %g s" % (
            fps,
            max_gap,
        )
        print(output_text)
