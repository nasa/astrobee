...
def test_values(
    values, job_id, value_names, output_dir, bag_file, map_file, image_topic, gnc_config
):
    ...
    options = ekf_graph.RunEKFOptions(
        bag_file, map_file, ekf_output_file, pdf_output_file, results_csv_output_file
    )
...
def make_values_and_parameter_sweep(
    output_dir, bag_file, map_file, image_topic, gnc_config
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-g",
        default=None,
        help="GNC config to use as a base for the parameter sweep.  Parameters not swept over will be taken from this file. Default will be gnc.config in astrobee/config",
    )
    ...
    make_values_and_parameter_sweep(
        args.directory, args.bag_file, args.map_file, args.image_topic, gnc_config
    )
