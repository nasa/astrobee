...
def initialize_environment(astrobee_map, astrobee_bag, astrobee_robot):
    ...
    try:
        ...
        print("Failed to find package astrobee.", file=sys.stderr)
        sys.exit(0)
    ...
    if not os.path.isfile(robot_config):
        print(
            "Please create the robot config file %s." % (robot_config), file=sys.stderr
        )
        sys.exit(0)
