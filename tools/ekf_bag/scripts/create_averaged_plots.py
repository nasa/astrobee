...
def get_subdirectory_files(file_string):
    ...
    _, subdirectories, _ = next(os.walk(os.getcwd()))
    for subdirectory in subdirectories:
        subdirectory_path = os.path.join(os.getcwd(), subdirectory)
        for subdirectory_csv_file in utilities.get_files(
            subdirectory_path, file_string
        ):
...
if __name__ == "__main__":
    ...
    plot_creator.create_pdf(
        dataframes, pdf_filename, True, os.path.basename(os.getcwd())
    )
