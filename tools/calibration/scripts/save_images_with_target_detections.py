    ...
    for timestamp, image in dataset.readDataset():
        ...
        if success:
            ...
            filepath = os.path.join(
                output_directory,
                os.path.splitext(os.path.basename(dataset.bagfile))[0]
                + "_"
                + str(timestamp.toSec()),
            )
...
def save_images_from_bags_directory_with_target_detections(
    bags_directory, target_yaml, cam_topic, output_directory, threshold
):
    ...
    for bag_name in bag_names:
        ...
        save_images_from_dataset_with_target_detections(
            dataset, target_detector.detector, args.output_directory, added_corners
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--directory",
        default="./",
        help="Directory of bagfiles containing images used for target detection.",
    )
    parser.add_argument(
        "-o", "--output-directory", default="./images_with_target_detections"
    )
    parser.add_argument(
        "--cam-topic", default="/hw/cam_nav", help="Camera topic in bagfiles."
    )
    parser.add_argument(
        "-t",
        "--target-yaml",
        help="Target yamf file specifying target parameters for target detection.",
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=100,
        help="Threshold to prevent overlapping images. If the mean difference norm in image space for a new image's target detection points compared with any already added target detections image are below the provided value, the image is ignored.  The two images must detect the exact same point set.",
    )
    args = parser.parse_args()
    save_images_from_bags_directory_with_target_detections(
        args.directory,
        args.target_yaml,
        args.cam_topic,
