...
class EkfBagVideo : public ekf_bag::EkfBag {
 public:
  EkfBagVideo(const char* bagfile, const char* mapfile, const char* videofile,
              bool run_ekf, bool gen_features, bool use_jem,
              const char* biasfile, std::string image_topic);
 ...
 private:
  void DrawFrame(void);
  void DrawImage(QPainter& p, const sensor_msgs::ImageConstPtr& ros_image,
                 const QRect& rect);
  void DrawPose(QPainter& p, const QRect& rect);
  ...
  void DrawTimestamp(QPainter& p, const QRect& rect);
  ...
  void DrawBarGraph(QPainter& p, const QRect& rect, int data_count,
