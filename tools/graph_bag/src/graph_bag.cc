...
void GraphBag::Run() {
  ...
  for (rosbag::MessageInstance const m : view) {
    if (string_ends_with(m.getTopic(), TOPIC_HARDWARE_IMU)) {
      ...
      // Handle of features before vl features since graph requires states to exist at timestamp
      // already when adding vl features, and these states are created when adding of features
      ...
      // Save latest graph pose, which should have just been optimized for after adding of and/or vl features
