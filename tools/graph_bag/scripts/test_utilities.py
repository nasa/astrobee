...
class TestRMSESequence(unittest.TestCase):
  ...
  def test_prune_missing_timestamps_beginning_set(self):
    ...
    pruned_a = utilities.prune_missing_timestamps(a_xs, a_ys, a_zs, a_times, b_times)
    self.assertEqual(len(pruned_a), len(b_times))
    self.assertTrue(np.allclose(pruned_a[:, 0], b_times))
    self.assertTrue(np.allclose(pruned_a[:, 1], b_times + 1.0))
    self.assertTrue(np.allclose(pruned_a[:, 2], b_times + 2.0))
  ...
  def test_rmse_matrix_ones(self):
    ...
    self.assertTrue(np.isclose(utilities.rmse_matrix(ones_mat), math.sqrt(3.0)))
  ...
  def test_rmse_matrix_1_2_2(self):
    ...
    self.assertTrue(np.isclose(utilities.rmse_matrix(mat), 3.0))
