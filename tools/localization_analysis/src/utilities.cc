  ...
  for (const auto& match : sem_matches) {
    ...
    if (match.have_map_point) {
      ...
      if (params.class_names.count(match.cls) > 0) {
        cv::putText(viz, params.class_names.at(match.cls), map_pt+(cv::Point(6, 6)/resize_scale),
            cv::FONT_HERSHEY_SIMPLEX, 0.5/resize_scale, cv::Scalar(0, 255, 0), 2/resize_scale);
      }
    }
  }
