...
parser.add_argument(
    ...
    "-d",
    type=str,
    help="full path to the directory that holds all the differnt bags from the issac activity/splices",
)
...
parser.add_argument(
    ...
    "-r",
    type=str,
    help="put the first letter of the robot your working with(queen, bumble, honey)",
)
...
parser.add_argument(
    ...
    "-f",
    action="store_true",
    help="To generate models for splices only"
...
def model_generator(path):
    for splice in os.listdir(path):
        ...
        try:
            ...
            run_colmap_command(["exhaustive_matcher", "--database_path", database_path])
