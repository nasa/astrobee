...
class CpuUsage:
    ...
    def printResult(self):
        ...
        for llp_node in self.llp_nodes:
            ax[1,0].plot(xpoints, self.llp_nodes[llp_node],
                label=llp_node, alpha=0.8)
        ...
        for mlp_node in self.mlp_nodes:
            ax[1,1].plot(xpoints, self.mlp_nodes[mlp_node],
                label=mlp_node, alpha=0.8)
        ...
        ax[2,0].stackplot(np.array(self.llp_nodes_time) - self.llp_nodes_time[0], self.llp_nodes.values(),
             labels=self.llp_nodes.keys(), alpha=0.8)
        ax[2,0].set_xlabel('Time(s)')
        ax[2,0].set_ylabel('Load')
        ax[2,0].set_title("Node CPU loads")
        ax[2,0].legend()

        ax[2,1].stackplot(np.array(self.mlp_nodes_time) - self.mlp_nodes_time[0], self.mlp_nodes.values(),
             labels=self.mlp_nodes.keys(), alpha=0.8)
...
if __name__ == "__main__":
    ...
    # Create dictionary
    freq_analyzer = dict ((topic, Topic(topic, i, len(args.topic_list))) for i, topic in enumerate(args.topic_list))
